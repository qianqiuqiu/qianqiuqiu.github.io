...
比如说Qwen2.5-1.5B本来是文本生成的模型，如果用AutoModelForSequenceClassification去下载它，就会是AutoModelForSequenceClassification:
┌─────────────────────┐
│   Qwen Transformer  │  ← 预训练好的（和上面一样）
├─────────────────────┤
│   Score Head (2)    │  ← 随机初始化的（num_labels）
└─────────────────────┘
原本用AutoModelForCausalLM会是:
┌─────────────────────┐
│   Qwen Transformer  │  ← 预训练好的
├─────────────────────┤
│   LM Head (151936)  │  ← 预训练好的（词表大小）
└─────────────────────┘
